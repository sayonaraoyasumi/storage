...
let aaa = "aaa";
console.log("DEBUG:スクロール処理 " + aaa );
		// 範囲が選択されている場合は処理しない
		const selection = window.getSelection();
		if( selection.toString().length ){ return; }
console.log("DEBUG:最初の関門超えた " + aaa );

		//	オプションバー表示されてたら閉じる
		if( jQuery( '.optionBar' ).hasClass( "show" ) ){
console.log("DEBUG:オプションバーが開いている " + aaa );
...
console.log("DEBUG:翻訳ウインドウ開いてる " + aaa );
